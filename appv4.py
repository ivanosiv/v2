...
# --- Função para carregar a interface HTML do chat realtime ---
def realtime_chat_interface():
    # Carrega o conteúdo do arquivo index.html (certifique-se de que ele esteja no mesmo diretório)
    try:
        with open("index.html", "r", encoding="utf-8") as f:
            html_content = f.read()
        # Exibe o HTML embutido na aplicação
        components.html(html_content, height=600, scrolling=True)
    except Exception as e:
        st.error(f"Erro ao carregar a interface realtime: {e}")
...
def main():
    ...
    # Se o usuário escolher "Chat Multimídia (Real-time)", exibe a interface HTML embutida
    if option == "Chat Multimídia (Real-time)":
        st.subheader("Chat Multimídia (Real-time)")
        st.info("Certifique-se de que o servidor WebSocket (por exemplo, a partir do 'main.py') esteja rodando.")
